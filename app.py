...
# ────────────────────────────────────────────────
#               تبويب الحجز الجديد (مع الحفاظ على البيانات عند الخطأ)
# ────────────────────────────────────────────────
with tabs[0]:
    # تهيئة الحقول في الجلسة إذا لم تكن موجودة
    defaults = {
        "name": "",
    ...
    with st.form("booking_form_final", clear_on_submit=False):
        ...
        with col1:
            ...
            name = st.text_input("", value=st.session_state["name"], key="name_input", label_visibility="collapsed")
            
            st.markdown("**تاريخ الازدياد** 📅")
            birth_date = st.date_input("", value=st.session_state["birth_date"], key="birth_date_input", label_visibility="collapsed")
            
            st.markdown("**مكان الازدياد** 🏙️")
            birth_place = st.text_input("", value=st.session_state["birth_place"], key="birth_place_input", label_visibility="collapsed")
            
            st.markdown("**العنوان الكامل** 🏠")
            address = st.text_input("", value=st.session_state["address"], key="address_input", label_visibility="collapsed")

        with col2:
            st.markdown("**الجنسية** 🌍")
            nationalities = ["الجزائر", "تونس", "المغرب", "فرنسا", "إسبانيا", "إيطاليا", "تركيا", "أخرى"]
            nationality = st.selectbox("", nationalities, index=nationalities.index(st.session_state["nationality"]) if st.session_state["nationality"] in nationalities else 0, key="nationality_select", label_visibility="collapsed")
            
            st.markdown("**نوع الوثيقة** 🪪")
            id_types = ["بطاقة التعريف الوطنية", "جواز السفر", "رخصة السياقة", "أخرى"]
            id_type = st.selectbox("", id_types, index=id_types.index(st.session_state["id_type"]) if st.session_state["id_type"] in id_types else 0, key="id_type_select", label_visibility="collapsed")
            ...
            id_number = st.text_input("", value=st.session_state["id_number"], key="id_number_input", label_visibility="collapsed")
            
            st.markdown("**رقم الهاتف** 📱 (اختياري)")
            phone = st.text_input("", value=st.session_state["phone"], placeholder="05XX XXX XXX", key="phone_input", label_visibility="collapsed")
        ...
        with c1:
            ...
            nights = st.number_input("", min_value=1, value=st.session_state["nights"], step=1, key="nights_input", label_visibility="collapsed")
        with c2:
            st.markdown("**تاريخ الدخول** 📅")
            check_in = st.date_input("", value=st.session_state["check_in"], key="check_in_input", label_visibility="collapsed")
        ...
        wing = st.selectbox("الجناح", list(wings.keys()), index=list(wings.keys()).index(st.session_state["wing"]) if st.session_state["wing"] in wings else 0, key="wing_select")
        room = st.selectbox("الغرفة", list(wings[wing].keys()), index=list(wings[wing].keys()).index(st.session_state["room"]) if st.session_state["room"] in wings[wing] else 0, key="room_select")
        ...
        with st.expander("ℹ️ معلومات إضافية وطلبات خاصة", expanded=False):
            ...
            purpose = st.selectbox("", purposes, index=purposes.index(st.session_state["purpose"]) if st.session_state["purpose"] in purposes else 0, key="purpose_select")
            if purpose == "أخرى":
                purpose_other = st.text_input("وضّح السبب الآخر", value=st.session_state["purpose_other"], key="purpose_other_input")

            companions_count = st.number_input("عدد الأشخاص المرافقين", min_value=0, max_value=8, value=st.session_state["companions_count"], key="companions_count")
            if companions_count > 0:
                companions_names = st.text_area(
                    f"أسماء المرافقين ({companions_count})",
                    value=st.session_state["companions_names"],
                    height=100,
                    placeholder="اكتب اسم كل شخص في سطر منفصل\nخاصة القاصرين",
                    key="companions_names_area"
                )

            notes = st.text_area("ملاحظات / طلبات خاصة", value=st.session_state["notes"], height=110,
        ...
        if age < 18:
            ...
            with colG1:
                guardian_name = st.text_input("اسم ولي الأمر / الوصي", value=st.session_state["guardian_name"], key="guardian_name_input")
            with colG2:
                ...
                ], index=0 if st.session_state["guardian_permission"] not in [...] else [...].index(st.session_state["guardian_permission"]), key="guardian_permission_select")
        ...
        if submitted:
            ...
            st.session_state["purpose_other"] = purpose_other if 'purpose_other' in locals() else ""
            st.session_state["companions_count"] = companions_count
            st.session_state["companions_names"] = companions_names if 'companions_names' in locals() else ""
            st.session_state["notes"] = notes
            st.session_state["guardian_name"] = guardian_name
            st.session_state["guardian_permission"] = guardian_permission
                ...
                st.markdown(
                    ...
                    <div class="success-box">
                        ...
                        <p style="margin:0.8rem 0; font-size:1.25rem; font-weight:bold; color:#006400;">
                            💰 التكلفة المتوقعة: {nights * 400:,} دج
                        </p>
                    </div>
                    """,
                    unsafe_allow_html=True
