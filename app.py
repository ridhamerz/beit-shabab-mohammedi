...
# ────────────────────────────────────────────────
#               تبويب الحجز الجديد (النسخة النهائية المحسنة)
# ────────────────────────────────────────────────
with tabs[0]:
    st.markdown(
    ...
    with st.form("booking_form_final", clear_on_submit=True):
        ...
        with col1:
            ...
            name = st.text_input("", placeholder="الاسم الكامل", key="name_input", label_visibility="collapsed")
            
            st.markdown("**تاريخ الازدياد** 📅")
            birth_date = st.date_input("", value=date.today() - timedelta(days=365*22), key="birth_date_input", label_visibility="collapsed")
            
            st.markdown("**مكان الازدياد** 🏙️")
            birth_place = st.text_input("", key="birth_place_input", label_visibility="collapsed")
            
            st.markdown("**العنوان الكامل** 🏠")
            address = st.text_input("", key="address_input", label_visibility="collapsed")

        with col2:
            st.markdown("**الجنسية** 🌍")
            nationalities = ["الجزائر", "تونس", "المغرب", "فرنسا", "إسبانيا", "إيطاليا", "تركيا", "أخرى"]
            nationality = st.selectbox("", nationalities, index=0, key="nationality_select", label_visibility="collapsed")
            ...
            id_type = st.selectbox("", [
                ...
                "رخصة السياقة",
                "أخرى"
            ], key="id_type_select", label_visibility="collapsed")
            ...
            id_number = st.text_input("", key="id_number_input", label_visibility="collapsed")
            
            st.markdown("**رقم الهاتف** 📱")
            phone = st.text_input("", placeholder="05XX XXX XXX", key="phone_input", label_visibility="collapsed")
        ...
        with c1:
            ...
            nights = st.number_input("", min_value=1, value=1, step=1, key="nights_input", label_visibility="collapsed")
        with c2:
            st.markdown("**تاريخ الدخول** 📅")
            check_in = st.date_input("", value=date.today(), key="check_in_input", label_visibility="collapsed")
        ...
        wing = st.selectbox("الجناح", list(wings.keys()), key="wing_select")
        room = st.selectbox("الغرفة", list(wings[wing].keys()), key="room_select")
        ...
        with st.expander("ℹ️ معلومات إضافية وطلبات خاصة", expanded=False):
            ...
            purpose = st.selectbox("", purposes, key="purpose_select")
            if purpose == "أخرى":
                purpose_other = st.text_input("وضّح السبب الآخر", key="purpose_other_input")

            companions_count = st.number_input("عدد الأشخاص المرافقين", min_value=0, max_value=8, value=0, key="companions_count")
        ...
        if age < 18:
            ...
            with colG1:
                guardian_name = st.text_input("اسم ولي الأمر / الوصي", key="guardian_name_input")
            with colG2:
                ...
                ], key="guardian_permission_select")
...
# باقي التبويبات (يمكن توسيعها لاحقًا)
with tabs[2]:
    st.subheader("📋 السجل الحالي")
    st.info("قسم السجل – يمكن إضافة عرض البيانات هنا")

with tabs[3] if len(tabs) > 3 else None:
    if len(tabs) > 3:
        st.subheader("📂 الأرشيف")
        st.info("قسم الأرشيف + تصدير")
