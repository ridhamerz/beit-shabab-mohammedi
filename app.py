...
DB_FILE = 'hostel_guelma_v15.db'
CAP_MALE, CAP_FEMALE = 60, 60

def init_db():
    with sqlite3.connect(DB_FILE) as conn:
        # إضافة عمود address هنا
        conn.execute('''CREATE TABLE IF NOT EXISTS guests (
            ...
            address TEXT, guardian TEXT, companions TEXT, wing TEXT, room TEXT, nights INTEGER, check_in TEXT, status TEXT DEFAULT 'مقيم'
        )''')
...
with tabs[0]:
    ...
    if st.session_state.step == "input":
        ...
        search_id = st.text_input("أدخل رقم الهوية للبحث:")
        found_data = None
        if search_id:
            with get_db_connection() as conn:
                # تحديث استعلام البحث ليشمل العنوان (address)
                found_data = conn.execute("SELECT name, bday, bplace, id_type, nationality, job, phone, guardian, companions, address FROM guests WHERE id_num=? ORDER BY id DESC LIMIT 1", (search_id,)).fetchone()
        ...
        with st.form("booking_form"):
            ...
            # إضافة حقل العنوان الشخصي هنا
            address = st.text_input("🏠 العنوان الشخصي الكامل", value=found_data[9] if found_data else "")
            ...
            if st.form_submit_button("📑 مراجعة الحجز", type="primary", use_container_width=True):
                ...
                if current_selection in occupied:
                    st.error(f"⚠️ الغرفة {room} مشغولة حالياً!")
                elif name and id_num:
                    st.session_state.temp = {"name":name, "bday":str(bday), "wing":wing, "room":room, "nights":nights, "id":id_num, "phone":phone, "job":job, "comps":comps, "address":address}
        ...
        st.info(f"النزيل: {d['name']} | العنوان: {d['address']} | المدة: {d['nights']} ليالٍ")
        
        if st.button("✅ تأكيد وحفظ", type="primary"):
            with get_db_connection() as conn:
                conn.execute("INSERT INTO guests (name, bday, wing, room, nights, id_num, phone, job, companions, address, check_in) VALUES (?,?,?,?,?,?,?,?,?,?,?)", 
                             (d['name'], d['bday'], d['wing'], d['room'], d['nights'], d['id'], d['phone'], d['job'], d['comps'], d['address'], str(date.today())))
