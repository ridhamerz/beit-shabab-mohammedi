...
st.markdown(f"""
    <div class="stat-container">
        ...
        <div class="stat-card" style="background:#f39c12"><span class="stat-label">⭐ حالة النظام</span><span class="stat-val">نشط</span></div>
    </div>
""", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    id_input = st.text_input("🔍 ابحث برقم الهوية لملء البيانات تلقائياً")
    ...
    if old_data: st.success(f"✅ تم العثور على بيانات النزيل سابقاً: {old_data[0]}")
    ...
    st.markdown("---")
    st.subheader("🛏️ خريطة اختيار السرير التفاعلية")
    ...
    if st.button("💾 حفظ الحجز في السجل", use_container_width=True, type="primary"):
        if not g_name or not id_input or not st.session_state.get('selected_bed'):
            st.error("⚠️ يرجى ملء الاسم، رقم الهوية واختيار السرير من الخريطة")
        else:
            try:
                with sqlite3.connect(DB_FILE) as conn:
                ...
                st.success("✅ تم التسجيل بنجاح"); st.session_state.selected_bed = None; st.rerun()
            except: st.error("🚫 هذا النزيل موجود فعلاً في القائمة الحالية!")
