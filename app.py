...
# ────────────────────────────────────────────────
with tabs[0]:
    # تهيئة القيم الافتراضية في الجلسة
    defaults = {
        "name": "",
    ...
    with st.form("booking_form_final", clear_on_submit=False):
        ...
        if age < 18:
            ...
            with colG2:
                gp_index = 0  # قيمة افتراضية
                if st.session_state["form_guardian_permission"] in ["موافقة خطية موقعة", "حضور ولي الأمر شخصياً", "إذن قضائي / وصاية رسمية", "مرافق معتمد من ولي الأمر", "حالة خاصة (يرجى التوضيح في الملاحظات)"]:
                    gp_index = ["موافقة خطية موقعة", "حضور ولي الأمر شخصياً", "إذن قضائي / وصاية رسمية", "مرافق معتمد من ولي الأمر", "حالة خاصة (يرجى التوضيح في الملاحظات)"].index(st.session_state["form_guardian_permission"])
                ...
                ], index=gp_index, key="guardian_permission_select")
                ...
                st.markdown(
                    ...
                    <div class="success-box">
                        ...
                        <p style="margin:0.5rem 0; font-size:1.1rem;">
                            <strong>النزيل:</strong> {current["name"]}
                        </p>
                        <p style="margin:0.5rem 0;">
                            <strong>المكان:</strong> {current["wing"]} → {current["room"]} → {current["bed"]}
                        </p>
                        <p style="margin:0.5rem 0;">
                            <strong>المدة:</strong> {current["nights"]} ليلة من {current["check_in"]:%Y-%m-%d} إلى {(current["check_in"] + timedelta(days=current["nights"])):%Y-%m-%d}
                        </p>
                        <p style="margin:0.8rem 0; font-size:1.25rem; font-weight:bold; color:#006400;">
                            💰 التكلفة المتوقعة: {current["nights"] * 400:,} دج
                        </p>
                    </div>
                    """,
                    unsafe_allow_html=True
