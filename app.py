...
if not st.session_state.authenticated:
    ...
    with col2:
        ...
        if st.button("🚀 تسجيل الدخول", use_container_width=True):
            ...
            if role == "مدير" and hashlib.sha256(pwd.encode()).hexdigest() == "8d969eef6ecad3c701f0e1f4c5f4d8e9f2e0e1f4c5f4d8e9f2e0e1f4c5f4d8e9f2":  # 159753
                st.session_state.authenticated = True
                st.session_state.role = role
                st.rerun()
            elif role == "عون استقبال" and hashlib.sha256(pwd.encode()).hexdigest() == "8d969eef6ecad3c701f0e1f4c5f4d8e9f2e0e1f4c5f4d8e9f2e0e1f4c5f4d8e9f2":  # 12345
                st.session_state.authenticated = True
                st.session_state.role = role
                st.rerun()
            else:
...
with tabs[2]:
    ...
    if not df_filtered.empty:
        selected_id = st.selectbox("اختر حجز للتعديل أو الحذف", df_filtered['id'].tolist())
...
with tabs[4]:
    ...
    st.info("سيتم إضافة خاصية إدارة الأفواج الكاملة قريبًا")
...
st.markdown(f'''
    <div class="developer-footer">
        ...
        📍 بيت شباب محمدي يوسف قالمة - النسخة المحسنة 2026 ✨
    </div>
    ''', unsafe_allow_html=True)
