...
st.markdown("""
    ...
    * { font-family: 'Cairo', sans-serif; direction: RTL; text-align: right; }
    ...
    .foreign-box { background: #fff3cd; padding: 15px; border-radius: 10px; border: 1px solid #ffeeba; margin-bottom: 15px; }
    .developer-footer { 
...
# ────────────────────────────────────────────────
#               2. إدارة قاعدة البيانات (تحديث الأعمدة)
# ────────────────────────────────────────────────
DB_FILE = 'youth_hostel_v11.db'
...
def init_db():
    with sqlite3.connect(DB_FILE) as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS current_guests (
            ...
            status TEXT DEFAULT 'مقيم', job TEXT, phone TEXT, notes TEXT, nights INTEGER
        )''')
        conn.commit()
...
# --- تبويب الحجز (التعديلات الجديدة هنا) ---
with tabs[0]:
    st.markdown('<div class="section-box"><h4>🔍 البحث السريع والتسجيل</h4></div>', unsafe_allow_html=True)
    search_id = st.text_input("🪪 أدخل رقم الوثيقة للبحث التلقائي:", placeholder="اكتب رقم بطاقة التعريف أو جواز السفر...")
    
    old_guest = get_old_guest(search_id) if search_id else None
    if old_guest: st.info(f"✅ تم العثور على بيانات سابقة للنزيل: {old_guest[0]}")

    with st.form("booking_v11"):
        c1, c2 = st.columns(2)
        with c1:
            ...
            nationality = st.text_input("🌍 الجنسية", value=old_guest[6] if old_guest else "جزائرية")

        with c2:
            id_type = st.selectbox("📄 نوع وثيقة الهوية", [
                "بطاقة تعريف (عادية)", 
            ...
            id_num = st.text_input("🔢 رقم الوثيقة المختارة", value=search_id if search_id else "")
            phone = st.text_input("📞 رقم الهاتف", value=old_guest[5] if old_guest else "")
            nights = st.number_input("🌙 عدد الليالي", min_value=1, value=1)
            
            # ميزة الأجانب (تاريخ الفيزا)
            is_foreigner = nationality.strip() not in ["جزائرية", "الجزائر", "Algerian", "Algerie"]
            entry_date = None
            if is_foreigner:
                st.markdown('<div class="foreign-box"><b>🛂 بيانات الأجانب:</b>', unsafe_allow_html=True)
        ...
        if vacant_beds:
            bed = st.radio("🛏️ الأسرة المتاحة حالياً:", vacant_beds, horizontal=True)
            notes = st.text_area("📝 ملاحظات إضافية")
            submit = st.form_submit_button("💾 تأكيد الحجز وتسجيل البيانات", type="primary", use_container_width=True)
        else:
            st.error("⚠️ هذه الغرفة ممتلئة تماماً! يرجى اختيار غرفة أخرى.")
            submit = False
        ...
        if submit:
            ...
            with sqlite3.connect(DB_FILE) as conn:
                ...
                (name, birth_date, birth_place, address, id_type, id_card, nationality, entry_date, wing, room, bed, check_in, check_out, job, phone, notes, nights) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (name, str(bday), bplace, addr, id_type, id_num, nationality, str(entry_date) if entry_date else None, wing, room, bed, str(date.today()), str(out_date), job, phone, notes, nights))
            st.success("🎉 تم تسجيل النزيل بنجاح!"); st.balloons()

# --- تبويب حالة الغرف (الرؤية البصرية) ---
with tabs[1]:
    st.markdown('<div class="section-box"><h4>📊 خريطة توزيع الأسرة الحالية</h4></div>', unsafe_allow_html=True)
    with sqlite3.connect(DB_FILE) as conn:
        booked = pd.read_sql_query("SELECT wing, room, bed, name, id_type FROM current_guests WHERE status='مقيم'", conn)
        ...
        for i in range(1, 11):
            ...
            with cols[(i-1)%5]:
                ...
                for b in range(1, 7):
                    ...
                    info_tip = f"النزيل: {is_occ['name'].iloc[0]}\nالوثيقة: {is_occ['id_type'].iloc[0]}" if not is_occ.empty else "شاغر"
                    st.markdown(f'<div class="bed-box {color}" title="{info_tip}">{b}</div>', unsafe_allow_html=True)
